...
class WebDocApplication:
    ...
    def register(self, name, description, plugin, patterns=None, routes=None):
        if name in self.docs.keys():
            raise DocExistsException(f'Web document {name} has been already registered by {self.plugin.name}')
...
class Document:
    ...
    def add_route(self, name):
        if name not in self.routes.keys():
            ...
            if route is None:
                raise UnknownRouteException(f'Route {name} could not be added.')
